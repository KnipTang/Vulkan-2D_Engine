...
void Texture::createTextureImage(Buffer& buffer)
{
    ...
    std::vector<char> texture = readFile(TEXTURE_PATH);
    ...
    createImage(texWidth, texHeight, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory, buffer);
...
void Texture::createImage(uint32_t width, uint32_t height, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, Buffer& buffer)
{
...
void Texture::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, Buffer& buffer)
{
...
void Texture::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, Buffer& bufferClass)
{
    VkCommandBuffer commandBuffer = bufferClass.beginSingleTimeCommands();
    ...
    bufferClass.endSingleTimeCommands(commandBuffer);
}
...
void Texture::createTextureImageView()
{
    ...
    if (vkCreateImageView(m_VkDevice, &viewInfo, nullptr, &textureImageView) != VK_SUCCESS) {
...
void Texture::createTextureSampler(VkPhysicalDevice& physicalDevice)
{
    ...
    if (vkCreateSampler(m_VkDevice, &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS) {
...
void Texture::destroy()
{
    ...
    vkDestroyImageView(m_VkDevice, textureImageView, nullptr);

    vkDestroyImage(m_VkDevice, textureImage, nullptr);
    vkFreeMemory(m_VkDevice, textureImageMemory, nullptr);
}
