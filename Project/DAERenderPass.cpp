...
void RenderPass::createRenderPass(const VkFormat& swapChainImageFormat, Depth depth)
{
...
void RenderPass::beginRenderPass(const DAECommandBuffer& buffer, VkFramebuffer currentBuffer, VkExtent2D extent)
{
...
void RenderPass::endRenderPass(const DAECommandBuffer& buffer)
{
	vkCmdEndRenderPass(buffer.getVkCommandBuffer());
}

void RenderPass::initialize(const VkDevice& device)
{
	m_VkDevice = device;
}
